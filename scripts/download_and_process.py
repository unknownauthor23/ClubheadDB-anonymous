...
def download_clip(video_url_or_id, source, start_time, end_time, output_path):
    ...
    MODIFIED: Uses a more robust command for Reddit and warns about bad URLs.
    """
    command = []
    
    if source == 'reddit':
        # --- NEW: Check for the incorrect direct media link ---
        ...
        # Modified command for Reddit: Removed specific format selection to let yt-dlp auto-select.
        command = [
...
def main():
    ...
    if not all(col in metadata.columns for col in required_cols):
        print(f"ERROR: '{METADATA_FILE}' must contain 'youtube_id' and 'source' columns.")
        return
