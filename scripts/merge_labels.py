...
ANNOTATIONS_FILE = Path('annotations/annotations.parquet')
# -------------------

def create_labels_from_parquet(frames_dir: Path, annotations_path: Path):
    """
    ...
    if not all_image_paths:
        raise FileNotFoundError(f"No .jpg images found in '{frames_dir}'. Please run the download script first.")
    ...
    print("\nStatus: Intermediate label creation complete.")
    print(f"The '{frames_dir}' directory now contains 'labels' subfolders with .txt files for every image.")


def main():
    parser = argparse.ArgumentParser(description="Create intermediate YOLO .txt label files in the 'frames' directory structure.")
    parser.add_argument('--frames_dir', type=Path, default=FRAMES_DIR, help="Path to the base directory containing swing frames.")
    parser.add_argument('--ann_file', type=Path, default=ANNOTATIONS_FILE, help="Path to the Parquet file with annotations.")

    args = parser.parse_args()

    try:
        create_labels_from_parquet(args.frames_dir, args.ann_file)
